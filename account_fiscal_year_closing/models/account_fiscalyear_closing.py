...
class AccountFiscalyearClosing(models.Model):
    ...
    def _default_year(self):
        ...
        if lock_date.month < int(company.fiscalyear_last_month) and lock_date.day < company.fiscalyear_last_day:
            fiscalyear = fiscalyear - 1
        return fiscalyear

    def _default_company_id(self):
        return self.env["res.company"]._company_default_get("account.fiscalyear.closing")
    ...
    _sql_constraints = [
        ("year_company_uniq", "unique(year, company_id)", _("There should be only one fiscal year closing for that year and " "company!")),
    ]
    ...
    def _compute_is_new_template(self):
        for record in self:
            ...
            record.is_new_template = record.closing_template_id.id != record.stored_template_id.id
    ...
    def _prepare_config(self, tmpl_config):
        ...
        return {
            ...
            "date": date,
            "journal_id": (tmpl_config.journal_id or self._get_default_journal(self.company_id).id),
    ...
    def _onchange_year(self):
        ...
        self.date_start = fields.Date.to_string(date_end - relativedelta(years=1) + relativedelta(days=1))
        ...
        self.write(
            {
                ...
                "move_config_ids": [(5,)] + [(0, 0, x._convert_to_write(x._cache)) for x in move_configs],
            }
        )
    ...
    def unlink(self):
        if any(x.state not in ("draft", "cancelled") for x in self):
            raise exceptions.UserError(_("You can't remove any closing that is not in draft or " "cancelled state."))
...
class AccountFiscalyearClosingConfig(models.Model):
    ...
    _sql_constraints = [
        ("code_uniq", "unique(code, fyc_id)", _("Code must be unique per fiscal year closing!")),
    ]
    ...
    def closing_type_get(self, account):
        ...
        closing_types = self.closing_type_ids.filtered(lambda r: r.account_type_id == account.user_type_id)
        if closing_types:
            ...
            for account in src_accounts:
                ...
                if closing_type == "balance":
                    ...
                    for partner in partners:
                        balance, move_line = account_map.move_line_partner_prepare(account, partner)
                        if move_line:
                            move_lines.append(move_line)
                else:
